...
export function renderZodiacWheel(container) {
  ...
  const spinBtn = document.getElementById('zodiacSpinBtn');
  const resultEl = document.getElementById('zodiacResult');
  const infoPanel = document.getElementById('zodiacInfoPanel');
  ...
  const engine = new WheelEngine('zodiacCanvas', {
    ...
    onResult: (w) => {
      ...
      const s = localizedSigns.find(z => z.name === name);
      resultEl.innerHTML = `<div class="result-winner zodiac-result"><span class="zodiac-result-symbol">${s?.symbol||'✨'}</span><span class="result-text">${name}</span><span class="zodiac-element">${s?.element||''} Sign</span></div>`;
    ...
    onSpinStart: () => { audioManager.init(); resultMode.reset(); resultEl.classList.remove('show'); infoPanel.classList.remove('show'); spinBtn.disabled = true; }
  });

  const cp = new CustomizationPanel(engine, { wheelName: 'zodiac' });
  cp.render('zodiacSidebar');
  spinBtn.onclick = () => engine.spin();
