...
const routes = {
  ...
  'yes-no-tarot': () => import(`./wheels/YesNoTarot.js?v=${ASSET_VERSION}`).then((m) => m.renderYesNoTarot),
...
function updateStaticUi(uiText) {
  const mappings = {
    ...
    navYesNoTarotLabel: getLocalizedRouteContent(currentLocale, 'yes-no-tarot').title,
    navYesNoOracleLabel: getLocalizedRouteContent(currentLocale, 'yes-no-oracle').title,
